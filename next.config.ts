...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    // Το 512 προστέθηκε επειδή έλειπε υποψήφιο ανάμεσα στο 384 και το 640:
    // εικόνες που χρειάζονται ~410–520px (οι κάρτες υπηρεσιών στα 384px του
    // desktop, η εικόνα του αθλητή στα 440px) αναγκάζονταν να πάρουν το 640w.
    // Το PageSpeed το ανέφερε ως 71 KiB σπατάλης σε desktop.
    imageSizes: [64, 128, 256, 384, 512],
  },

  experimental: {
    // Το μοναδικό render-blocking αίτημα της σελίδας ήταν το φύλλο στυλ:
    // 13,6 KiB που κρατούσαν την πρώτη εμφάνιση πίσω κατά ~150 ms, επειδή ο
    // browser πρέπει να κατεβάσει το HTML, να βρει το <link> και μετά να
    // ζητήσει το CSS.
    //
    // Με Tailwind (atomic CSS) το αρχείο είναι μικρό και σχεδόν όλο σε χρήση,
    // που είναι ακριβώς η περίπτωση όπου η τεκμηρίωση του Next συνιστά
    // inlining. Το κόστος είναι ότι οι επαναλαμβανόμενοι επισκέπτες δεν
    // ξαναχρησιμοποιούν cached stylesheet — για ένα marketing site που
    // κρίνεται στην πρώτη επίσκεψη, το ανταλλάγμα συμφέρει.
    inlineCss: true,
  },
